...
export class RegistroAutologaComponent {
  ...
  buscar(): void {
    const busquedaTrim = this.busqueda?.trim();
    if (!busquedaTrim) {
      Swal.fire('Campo vacío', 'Por favor, ingrese un Número de DNI o Cuna para buscar.', 'info');
      return;
    }
    ...
    this.registroService.getAll().subscribe({
      next: (registros) => {
        ...
        const busquedaUpper = busquedaTrim.toUpperCase();

        let madrePorDni: any = null;
        let madrePorCuna: any = null;

        for (const reg of registros) {
          const madre = reg.madre;
          const paciente = madre?.paciente;
          const cuna = paciente?.cuna;

          // Buscar por DNI exacto (sin transformar el campo)
          if (madre?.idMadre?.toLowerCase() === busquedaLower) {
            madrePorDni = madre;
            break;
          }

          // Buscar por Cuna (C-01, I-02, etc.), mayúscula
          if (
            cuna?.idCuna?.toUpperCase() === busquedaUpper &&
            paciente?.estado === 'Paciente en atención'
          ) {
            madrePorCuna = madre;
          }
        }
        ...
        if (madrePorDni) {
          this.procesarMadreEncontrada(madrePorDni);
        } else if (madrePorCuna) {
          this.procesarMadreEncontrada(madrePorCuna);
        } else {
    ...
    this.registroService.getMadreById(idMadreOrCuna).subscribe({
      next: (madre) => {
        if (madre) {
          ...
          Swal.fire(`Datos no encontrados para ${idMadreOrCuna}`, 'Asegúrese de que la Madre que desea buscar esté registrada', 'warning');
          this.limpiar();
        }
      },
      error: (err) => {
  ...
  guardar(): void {
    if (!this.busqueda.trim() || this.madreEncontrada == null) {
